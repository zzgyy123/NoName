...
public class PlayerController2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetButtonDown(ZYGame.Input.JUMP))
        {
            _motor.Jump();
            _motor.DisableRestrictedArea();
        }

        _motor.jumpingHeld = Input.GetButton(ZYGame.Input.JUMP);

        // XY freedom movement
        if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
        {
            _motor.normalizedXMovement = Input.GetAxis(ZYGame.Input.HORIZONTAL);
            _motor.normalizedYMovement = Input.GetAxis(ZYGame.Input.VERTICAL);

            return; // do nothing more
        }

        // X axis movement
        if (Mathf.Abs(Input.GetAxis(ZYGame.Input.HORIZONTAL)) > ZYGame.Globals.INPUT_THRESHOLD)
        {
            _motor.normalizedXMovement = Input.GetAxis(ZYGame.Input.HORIZONTAL);
        }
        else
        ...
        if (Input.GetAxis(ZYGame.Input.VERTICAL) != 0)
        {
            bool up_pressed = Input.GetAxis(ZYGame.Input.VERTICAL) > 0;
        ...
        else if (Input.GetAxis(ZYGame.Input.VERTICAL) < -ZYGame.Globals.FAST_FALL_THRESHOLD)
        {
            _motor.fallFast = false;
        }

        if (Input.GetButtonDown(ZYGame.Input.DASH))
        {
            _motor.Dash();
        }
    }
